...
describe('StringUtils', () => {
    describe('removeLinkFromErrorMessageText', () => {
        it('removes link by classname from text', () => {
            ...
            const fixedText = removeLinkFromErrorMessageText(messageText, 'embedded-cta-link');
            expect(fixedText).toEqual(`You've filled the queue for your @hamstu Twitter account, nice work! Upgrading to any paid plan will unlock more space. `);
        });
        it('ignores links that do not match classname', () => {
            ...
            const fixedText = removeLinkFromErrorMessageText(messageText, 'embedded-cta-link');
            expect(fixedText).toEqual(`You've filled the queue for your @hamstu Twitter account, nice work! Upgrading to any paid plan will unlock more space. <a class="embedded-cta-link-diff" href="foo">bar</a>`);
        });
