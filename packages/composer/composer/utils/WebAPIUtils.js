...
import API from './API';
import { observeStore } from './StoreUtils';
import { extractSavedUpdatesIdsFromResponses } from './APIDataTransforms';
import { getComposerSource, getSegmentMetadata } from './TrackingUtils';

import getFacebookAutocompleteEntities from './draft-js-custom-plugins/autocomplete/utils/getFacebookAutocompleteEntities';

class APIRequestError extends Error {
  name = 'APIRequestError';
}

const getDraftsDataToSave = ({
  customScheduleTime,
  drafts,
  partiallySavedDraftsProfilesIds,
  profiles,
  queueingType,
  updateId,
}) => {
...
const splitDraftDataToSave = ({ draftsDataToSave }) => {
...
const splitSaveDraftResponses = responses => {
  /**
  ...
  const [unfilteredSuccessfulResponses, unsuccessfulResponses] = partition(
    responses,
    response => response.success
  );

  const [partiallySuccessfulResponses, successfulResponses] = partition(
    unfilteredSuccessfulResponses,
    ({ serviceName: successfulServiceName }) =>
      unsuccessfulResponses.find(
        ({ serviceName: unsuccessfulServiceName }) =>
          unsuccessfulServiceName === successfulServiceName
      )
  );
...
const updatePartiallySavedDraftsById = ({
  partiallySuccessfulResponses,
  successfulResponses,
  unfilteredSuccessfulResponses,
  unsuccessfulResponses,
...
const mapDraftsToApiFetch = ({
  ...
  ].includes(queueingType);

  return API.post(
    `updates/${updatePost ? `${updateId}/update.json` : 'create.json'}`,
    formattedData
  )
    .catch(() => ({
      success: false,
...
const mergeFinalResponses = ({
  ...
  const mergedSuccessfulResponses = successfulResponses.reduce(
    (mergingSuccessfulResponses, response) => {
      const existingServiceEntry = mergingSuccessfulResponses.find(
        mergedResponse => mergedResponse.serviceName === response.serviceName
      );

      if (existingServiceEntry) {
        existingServiceEntry.updates.splice(-1, 0, ...response.updates);
      } else {
        mergingSuccessfulResponses.push(response);
      }

      return mergingSuccessfulResponses;
    },
    []
  );

  // Merge unsuccessful responses back together again into one response per service
  const mergedUnsuccessfulResponses = unsuccessfulResponses.reduce(
    (mergingUnsuccessfulResponses, response) => {
      const hasExistingServiceEntry = !!mergingUnsuccessfulResponses.find(
        mergedResponse => mergedResponse.serviceName === response.serviceName
      );

      if (!hasExistingServiceEntry) {
        mergingUnsuccessfulResponses.push(response);
      }

      return mergingUnsuccessfulResponses;
    },
    []
  );
  ...
  return {
    partiallySuccessfulResponses,
    successfulResponses: mergedSuccessfulResponses,
    unfilteredSuccessfulResponses,
    unsuccessfulResponses: mergedUnsuccessfulResponses,
  };
};

const trackSavedDraftSuccessfulResponses = ({
  partiallySuccessfulResponses,
  successfulResponses,
  unfilteredSuccessfulResponses,
  unsuccessfulResponses,
...
const trackSavedDraftUnsuccessfulResponses = ({
  partiallySuccessfulResponses,
  successfulResponses,
  unfilteredSuccessfulResponses,
  unsuccessfulResponses,
...
const WebAPIUtils = {
   ...
   * }
   */
  saveDrafts: ({ customScheduleTime, drafts, profiles, queueingType }) => {
    ...
    const draftsDataToSave = getDraftsDataToSave({
      customScheduleTime,
      drafts,
      partiallySavedDraftsProfilesIds,
      profiles,
    ...
    const splitDraftsDataToSave = splitDraftDataToSave({
      draftsDataToSave,
    });
    ...
    return Promise.all(savePromises)
      .then(splitSaveDraftResponses)
      .then(updatePartiallySavedDraftsById)
      .then(mergeFinalResponses)
      .then(response => trackSavedDraftSuccessfulResponses({ ...response, queueingType}))
      .then(trackSavedDraftUnsuccessfulResponses)
      .then(
        ({
          partiallySuccessfulResponses,
          successfulResponses,
