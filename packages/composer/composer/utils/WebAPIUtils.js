...
const WebAPIUtils = {
  ...
  saveDrafts: ({ queueingType, customScheduleTime, profiles, drafts }) => {
      ...
      formattedData: getFormattedAPIData(draft.service.name, {
        ...
        serviceProfiles: profiles.filter((profile) => {
          const wasUpdateSavedAlreadyForProfile =
            (partiallySavedDraftsProfilesIds.get(draft.service.name) || []).includes(profile.id);

          return (profile.service.name === draft.service.name && !wasUpdateSavedAlreadyForProfile);
        }),
        serviceDraft: draft,
    ...
    const splitDraftsDataToSave =
      [].concat(...draftsDataToSave.map((draft) => (
          ...
          draft.formattedData.profile_ids
            .map((_, i) => (
              i % maxProfilesIdsPerRequest === 0 ?
                draft.formattedData.profile_ids.slice(i, i + maxProfilesIdsPerRequest) :
                null
            ))
            .filter((profilesIdsGroup) => profilesIdsGroup !== null)
            .map((profilesIdsGroup) => ({
              ...draft,
              formattedData: { ...draft.formattedData, profile_ids: profilesIdsGroup },
            }))
      )));
      ...
      return API.post(endpoint, formattedData)
        ...
        .then((response) => {
          if (!updatePost) {
            ...
            const composerSource = getComposerSource({ tabId, emptySlotMode, profile });
    ...
    return Promise.all(savePromises)
      .then((responses) => {
        ...
        let [unfilteredSuccessfulResponses, unsuccessfulResponses] =
          partition(responses, (response) => response.success);

        // eslint-disable-next-line prefer-const
        let [partiallySuccessfulResponses, successfulResponses] =
          partition(unfilteredSuccessfulResponses, (successfulResponse) => (
            unsuccessfulResponses.find((unsuccessfulResponse) =>
              unsuccessfulResponse.serviceName === successfulResponse.serviceName)
          ));
        ...
        successfulResponses = successfulResponses.reduce((mergedSuccessfulResponses, response) => {
          const existingServiceEntry = mergedSuccessfulResponses.find((mergedResponse) => (
        ...
        unsuccessfulResponses =
          unsuccessfulResponses.reduce((mergedUnsuccessfulResponses, response) => {
            const hasExistingServiceEntry = !!mergedUnsuccessfulResponses.find((mergedResponse) => (
              mergedResponse.serviceName === response.serviceName
            ));
        ...
        return {
          successfulResponses,
          unsuccessfulResponses:
            overrideAPIErrorResponseMessages({
              unsuccessfulResponses,
  ...
  return API.get('search/facebook_pages.json', { q: search, profile_id: selectedFbPages[0].id })
    ...
    .then((results) => (
      results.map(({ name, id, link: url }) => [
        id,
        { avatar: 'avatar', name, id, category: formatPageUrlAsUsername(url), url },
      ])
    ));
}
...
function overrideAPIErrorResponseMessages({
  ...
  return unsuccessfulResponses.map((response) => {
    ...
    const hasPartiallySuccessfulResponses =
      !!partiallySuccessfulResponses.find(({ serviceName }) => serviceName === response.serviceName);
    ...
    if (hasPartiallySuccessfulResponses) {
      ...
      response.message =
        `Some updates were successfully ${errorMessageVerbQueueingTypeMap.get(queueingType)},
