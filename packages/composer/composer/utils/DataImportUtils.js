...
const DataImportUtils = {
  ...
  formatProfilesData: (dataImportEnv, { profiles, subprofileId }, formattedMetaData) => {
    const data = [];
    ...
    profiles.forEach((importedProfile) => {
      ...
      const isTwitterPostingRestrictedForProfile = (
        ...
        !isEditingUpdate &&
        formattedMetaData.existingUpdateProfileService === importedProfile.service &&
      ...
      data = Object.assign(
        {
          ...
          hasAccessToHashtagManager: userData.hasAccessToHashtagManager,
  ...
  formatMetaData: (dataImportEnv, { metaData, update, scheduledAt, isPinnedToSlot }) => {
    ...
    switch (dataImportEnv) {
      ...
      case DataImportEnvironments.WEB_DASHBOARD: {
        const metaScheduledAt = scheduledAt || update.due_at || update.scheduled_at || null;
        ...
        const isPrefillingExistingUpdate = update.id && Object.keys(update).length > 0;

        meta = Object.assign({}, metaData, {
          isPrefillingExistingUpdate,
          existingUpdateProfileService: isPrefillingExistingUpdate ? update.profile_service : null,
