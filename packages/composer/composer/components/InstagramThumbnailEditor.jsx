...
import { addBlobIfNeeded, resizeImageIfNeeded, isSafari, isEdge } from '../utils/DOMUtils';
...
class InstagramThumbnailEditor extends React.Component {
  ...
  setThumbnail = (draftId) => {
    ...
    const { width, height } =
      resizeImageIfNeeded(maxSize, { width: video.videoWidth, height: video.videoHeight });
    canvas.width = width;
  ...
  createUploadCanvasFile = (canvas, video, draftId, sizeObj) => {
    ...
    canvas.toBlob((blob) => {
      ...
      ComposerActionCreators.uploadInstagramDraftThumbnail(draftId, imageFile, video);
    });
  }
    ...
    return (
      <div id="instagramThumbnail">
        <Modal
          ...
          hideCloseButton
          modalCustumStyle={modalDynamicStyles}
        >
          <div style={cardWrapperStyle}>
            <Card noPadding>
              <div style={{ padding: '0px 10px' }}>
                <div style={{ padding: '15px 0px 7px' }}>
                  <Text>Select a frame from your video to save as your thumbnail.</Text>
                </div>
